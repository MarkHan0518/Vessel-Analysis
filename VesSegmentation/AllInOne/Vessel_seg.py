"""
This module contains all the code needed for the vessel segmentation pipeline, including data preprocessing
(signal intensity clipping, background noise removal, edge preserving and smoothing, intensity normalization),
thresholding based algorithm (multi-otsu thresholding, hysteresis thresholding, chunk edge smoothing), and data post-processing
(hole filling and median filter smoothing). The user needs to define filepaths and parameters at the beginning of the main script.
"""
...
if __name__ == '__main__':
    ...
    ##### ---------- Chunk Edge Smoothing ---------- #####
    # CAVEAT: For a large dataset, the computer may run out of memory up to this point. Do it in a separate python task!!!
    divide_3d = (np.divide(otsu_3d, count_3d)).astype(np.uint8)
    # imsave(os.path.join(SAVEPATH, 'divide_3d.tif'), divide_3d)
    ##############################################


    ##### ---------- Hysteresis Thresholding ---------- #####
    # CAVEAT: For a large dataset, the computer may run out of memory up to this point. Do it in a separate python task!!!
    hyst = filters.apply_hysteresis_threshold(divide_3d, HYST_LOW, HYST_HIGH).astype(np.uint8)
    imsave(os.path.join(SAVEPATH, 'hyst.tif'), hyst)
    #########################################################
    ...
    imsave(os.path.join(SAVEPATH, 'hole_filling.tif'), hole_filling)
    ##############################################
    ...
    imsave(os.path.join(SAVEPATH, 'remove_region.tif'), remove_region)
    ...
    # CAVEAT: For a large dataset, the computer may run out of memory up to this point. Do it in a separate python task!!!
    size = 3
    smoothing = median_filter(remove_region, size=size)
    imsave(os.path.join(SAVEPATH, 'smoothing.tif'), smoothing)
    ##########################################