"""
This module contains all the code needed for the vessel segmentation pipeline.
...
def keep_largest_region(image) -> np.ndarray:
...
if __name__ == '__main__':
    ...
    object_min_size = 40 # minimum size of the object
    ###################################################
    ...
    ##### ---------- Keep the largest region (optional) ---------- #####
    # remove_start = time.time()
    # largest_region = keep_largest_region(hole_filling)
    # largest_region = largest_region.astype(np.uint8)
    # imsave(os.path.join(SAVEPATH, 'largest_region.tif'), largest_region)
    ...
    # remove_end = time.time()
    # remove_duration = remove_end - remove_start
    # print("Keep largest region time: " + str(round(remove_duration)) + ' s')
    ######################################################
